...
describe('Instantiate Step 2', () => {
  test('renders correctly with initial values', () => {
    const { getByText } = render(
      <InstantiateStep2 keyringPairs={keyringPairsMock} dispatch={jest.fn()} currentStep={2} />
    );
    expect(getByText('ALICE')).toBeInTheDocument();
  });
  test('does not render when current step is not 2', () => {
    const { container } = render(
      <InstantiateStep2 keyringPairs={keyringPairsMock} dispatch={jest.fn()} currentStep={1} />
    );
  ...
  test('dispatches the correct values', () => {
    ...
    const { getByText } = render(
      <InstantiateStep2 keyringPairs={keyringPairsMock} dispatch={dispatchMock} currentStep={2} />
    );
