...
export function saveInLocalStorage (contract: ContractPromise) {
...
export function getInstancesFromStorage () {
  const storedInstances = localStorage.getItem('contracts');
  return storedInstances ? (JSON.parse(storedInstances) as ContractPromise[]) : [];
};

export function getInstanceFromStorage (
  addr: string,
  api: ApiPromise | null
...
export function convertMetadata (metadata: unknown, api: ApiPromise | null) {
