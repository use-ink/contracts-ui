...
const Title = ({ parentType }: TitleProps) => {
  return <>{parentType}</>;
};

export function Vector({
  Component,
  props: { value, onChange, nestingNumber, parentType },
  props,
}: Props) {
  ...
  return (
    <div
      className={`pl-2 text-left text-sm ${
        isOddNesting ? 'bg-transparent' : 'bg-gray-500'
      } rounded border dark:border-gray-500 border-gray-200`}
    >
      <Title parentType={parentType} />
