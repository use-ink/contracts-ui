...
): [Record<string, unknown>, Registry, ContractMetadataSupported, ContractProjectInfo] {
...
function isTypeSpec(value: Codec): value is ContractTypeSpec {
  return (
    ...
    !isUndefined((value as ContractTypeSpec).type) &&
    !isUndefined((value as ContractTypeSpec).displayName)
  );
}
...
export class Abi {
  ...
  constructor(abiJson: Record<string, unknown> | string, chainProperties?: ChainProperties) {
    [this.json, this.registry, this.metadata, this.info] = parseJson(
      isString(abiJson) ? (JSON.parse(abiJson) as Record<string, unknown>) : abiJson,
      chainProperties,
    );
    this.constructors = this.metadata.spec.constructors.map(
      (spec: ContractConstructorSpecLatest, index) =>
    ...
    this.messages = this.metadata.spec.messages.map(
      (spec: ContractMessageSpecLatest, index): AbiMessage =>
        this.#createMessage(spec, index, {
          isDefault: spec.default.isTrue,
  ...
  #createArgs = (
    args: ContractMessageParamSpecLatest[] | ContractEventParamSpecLatest[],
    spec: unknown,
  ): AbiParam[] => {
  ...
  #createMessageParams = (
    args: ContractMessageParamSpecLatest[],
    spec: unknown,
  ): AbiMessageParam[] => {
  ...
  #createEventParams = (args: ContractEventParamSpecLatest[], spec: unknown): AbiEventParam[] => {
    ...
    return params.map(
      (p, index): AbiEventParam => ({ ...p, indexed: args[index].indexed.toPrimitive() }),
    );
  };
  ...
  #createMessage = (
    spec: ContractMessageSpecLatest | ContractConstructorSpecLatest,
    index: number,
    add: Partial<AbiMessage> = {},
  ): AbiMessage => {
    ...
    const message = {
      ...
      toU8a: (params: unknown[]) => this.#encodeMessageArgs(spec, args, params),
    };
  ...
  #encodeMessageArgs = (
    { label, selector }: ContractMessageSpecLatest | ContractConstructorSpecLatest,
